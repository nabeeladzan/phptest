					...
					if(isset($_POST['submit'])){
						...
						if(!in_array($type2, $tipe_diizinkan)){
							...
							function fileWriteAppend(){
								$current_data = file_get_contents('produk.json');
								...
								$array_data[] = $extra;
								$final_data = json_readable_encode($array_data);
								return $final_data;
							}

							function fileCreateWrite(){
								$file=fopen("produk.json","w");
								$array_data=array();
								$extra = array(
									 'kategori'		=>	$_POST['kategori'],
									 'nama'			=>	$_POST['nama'],
							...
							if(file_exists('produk.json'))
							{
								 $final_data=fileWriteAppend();
								 if(file_put_contents('produk.json', $final_data))
								 {
